...
Flux.train!((m,x,y) -> (m(x) - y)^2, model, data, opt_state; epochs=100)

using Plots
plot(x -> 2x-x^3, -2, 2, label="truth")
scatter!(model, -2:0.1f0:2, label="learned")
```
In Flux v0.15 and later, almost any parameterised function in Julia is a valid Flux model -- such as this closure over `w, b, v`. The same function can also be implemented with built-in layers as shown.
